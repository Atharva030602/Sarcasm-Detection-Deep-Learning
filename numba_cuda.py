...
subprocess.check_call(["find", "/", "-iname", "libdevice"])
subprocess.check_call(["find", "/", "-iname", "libnvvm.so"])
...
r"""It's often useful to convert cartesian coordinates into the polar coordinates `R` and `theta`, where r is the distance of the point to origin, and where theta is the angle between the x axis and the direction of the point. 

Diagram: https://raw.githubusercontent.com/cbernet/maldives/master/numba/cart_polar.png
...
r"""Because of the correlation between x and y, we see two peaks at $\pi/4$ and $-3\pi/4$.
